class StoryQuery < ApplicationQuery
  ...
  def self.base_relation
    relation_class.includes([:author, :rich_text_description, :chapters, { cover_image_attachment: :blob }, :primary_genre, :secondary_genre]).order(updated_at: :desc, id: :desc).with_published
  end
